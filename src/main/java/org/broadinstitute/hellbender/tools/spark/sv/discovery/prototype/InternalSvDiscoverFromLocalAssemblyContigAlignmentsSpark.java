...
public final class InternalSvDiscoverFromLocalAssemblyContigAlignmentsSpark extends GATKSparkTool {
    ...
    private static boolean isLikelyInvBreakpointOrInsInv(final AlignedContig contigWithOnlyOneConfigAnd2AlnToSameChr) {
        Utils.validateArg(hasOnly2Alignments(contigWithOnlyOneConfigAnd2AlnToSameChr),
                "assumption that input contig has only 2 alignments is violated. \n" +
                        onErrorStringRepForAlignedContig(contigWithOnlyOneConfigAnd2AlnToSameChr));
        final AlignmentInterval intervalOne = contigWithOnlyOneConfigAnd2AlnToSameChr.alignmentIntervals.get(0),
                                intervalTwo = contigWithOnlyOneConfigAnd2AlnToSameChr.alignmentIntervals.get(1);
        Utils.validateArg(intervalOne.referenceSpan.getContig().equals(intervalTwo.referenceSpan.getContig()),
    ...
    private static boolean isSuggestingRefBlockOrderSwitch(final AlignedContig contigWithOnlyOneConfigAnd2AlnToSameChrWithoutStrandSwitch) {
        Utils.validateArg(hasOnly2Alignments(contigWithOnlyOneConfigAnd2AlnToSameChrWithoutStrandSwitch),
                "assumption that input contig has only 2 alignments is violated. \n" +
        ...
        Utils.validateArg(intervalOne.forwardStrand == intervalTwo.forwardStrand,
                "assumption that input contig's 2 alignment map to the same strand is violated. \n" +
                        onErrorStringRepForAlignedContig(contigWithOnlyOneConfigAnd2AlnToSameChrWithoutStrandSwitch));
        Utils.validateArg(intervalOne.referenceSpan.getContig().equals(intervalTwo.referenceSpan.getContig()),
                "assumption that input contig's 2 alignments map to the same chr is violated. \n" +
                        onErrorStringRepForAlignedContig(contigWithOnlyOneConfigAnd2AlnToSameChrWithoutStrandSwitch));
    ...
    private static boolean isLikelyCpx(final AlignedContig contigWithOnlyOneConfig) {
        Utils.validateArg(!contigWithOnlyOneConfig.hasEquallyGoodAlnConfigurations,
                "assumption that input contig has one unique best alignment configuration is violated: " + contigWithOnlyOneConfig.contigName);

        return !hasOnly2Alignments(contigWithOnlyOneConfig) || !isSameChromosomeMapping(contigWithOnlyOneConfig);
    }

    private static EnumMap<RawTypes, JavaRDD<AlignedContig>> splitReadsByPossiblyRawTypes(final JavaRDD<AlignedContig> contigsWithAlignmentsReconstructed,
